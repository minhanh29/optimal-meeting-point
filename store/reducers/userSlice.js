...
export const signUpAsync = createAsyncThunk('user/signUpAsync', async (date)=>{
    // create an account on firebase auth
    const res = await createUserWithEmailAndPassword(auth, data.username, data.password)

    //save to Firestore
    await createUser(res.user.uid, data.address, data.avaUrl, data.gps_enabled, data.name, data.username)
